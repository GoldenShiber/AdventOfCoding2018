...
def collectNodeData(text):
    comma = findIndex(text, ",")
    xcoord = text[0:comma]
    ycoord = text[comma+2:len(text)-1]
...
def createNodeList(filepath):
    ...
    with open(filepath) as fp:
        for cnt in range(lengthOfFile):
            line = fp.readline()
            coordinates = collectNodeData(line)
...
def countArea(playmat, nodelist):
    ...
    for l in range(len(countList)):
        if countList[l] < 10000:
            regionSize = regionSize + countList[l]
